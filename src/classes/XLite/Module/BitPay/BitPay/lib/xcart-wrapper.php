...
class XcartWrapper {
	 ...
	 */
	public function __construct(\XLite\Model\Payment\Method $method) {
		$this->method = $method;
	...
	public function getPairingUrl() {
		// Sanitize label
        $name_entry = \XLite\Core\Database::getRepo('XLite\Model\Config')->findOneBy(array('name' => 'company_name'));
		$label      = preg_replace('/[^a-zA-Z0-9 \-\_\.]/', '', $name_entry['value']);
	...
	public function setting($key, $value = null) {
		...
		// Set the setting
		if (func_num_args() === 2) {
			return $this->method->setSetting($key, $value);
		}
